...
- 基于错误的剪枝(EBP)
    ...
    ![基于错误剪枝](img/9.png){width=50%}  
    训练的具体步骤：
    ![基于错误剪枝](img/10.png){width=50%} 
- 代价-复杂度剪枝(CCP)
    ![ccp](img/11.png){width=50%} 
    其中T为决策树，$C(T)$为预测误差 ， $ | T |$是树叶节点个数，$\alpha$ 代表复杂度，用来平衡拟合能力和泛化能力。
    具体步骤：
    ![ccp](img/12.png){width=50%} 

### CART算法 
#### 基尼指数
![基尼指数](img/13.png){width=50%} 
如何基于基尼指数来进行判别：
![分类](img/14.png){width=50%} 
**基尼指数越小，分类后的结果更加确定，分类效果越好**
#### 具体算法
##### 分类树
![分类](img/15.png){width=50%} 
##### 回归树
由于回归任务是连续的，所以必须切分成多个块才可以使用决策树。
![回归](img/16.png){width=50%} 
![回归](img/17.png){width=50%} 
具体算法流程
![回归](img/18.png){width=50%} 
#### 剪枝
利用ccp（代价-复杂度剪枝）来进行剪枝
算法流程：
![剪枝](img/19.png){width=50%} 
